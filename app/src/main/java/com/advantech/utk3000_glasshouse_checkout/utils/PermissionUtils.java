...
public class PermissionUtils {
	...
	public static boolean requestRuntimePermission(Activity activity, String[] permissions,
		...
		if (permissions != null && permissions.length > 0) {
			...
			if (rumtimePermissionList.size() > 0) {
				...
				ActivityCompat.requestPermissions(activity, REQUEST_RUNTIME_PERMISSIONS, requestCode);
			}
		}
	...
	public static boolean checkRequestRuntimePermissions(Context context) {
		...
		try {
			...
			Log.e(TAG, "Request all runtime permission failed.", e);
		}
